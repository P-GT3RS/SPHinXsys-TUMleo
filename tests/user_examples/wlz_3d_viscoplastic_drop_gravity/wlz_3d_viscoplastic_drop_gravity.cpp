...
//----------------------------------------------------------------------
Real resolution_ref = 0.025;      /* 粒子间距 */
Vec3d ball_center(0.0, 0.0, 2.0); /* 圆心位置距在X-Y平面上2.0处 */
BoundingBox system_domain_bounds(Vec3d(-2.5, -2.5, -0.5), Vec3d(2.5, 2.5, 3.0));
...
class WallBoundary : public ComplexShape
{
  public:
    ...
    {
        Vec3d halfsize_wall(1.5, 1.5, 0.1);     /* 地面长的一半,宽的一半,厚度的一半. 地面有0.1*2/0.025=8层. */
        Vec3d translation_wall(0.0, 0.0, -0.1); /* 地面中间层向Z轴负方向移动0.1 */
        add<TriangleMeshShapeBrick>(halfsize_wall, resolution_ref, translation_wall);
    }
};
//----------------------------------------------------------------------
...
int main(int ac, char *av[])
{
    ...
    DampingWithRandomChoice<InteractionSplit<DampingPairwiseInner<Vecd>>>
        damping(0.5, ball_inner, "Velocity", physical_viscosity);
    //----------------------------------------------------------------------
    ...
    write_ball_displacement.testResult();
    return 0;
}

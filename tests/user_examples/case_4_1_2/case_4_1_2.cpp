/**
 * @file 	4.1.2 Ball Collision with Cylinder.cpp
 * @details one ball with viscoplastic or oobleck (shear thickening) material collides with two cylinders.
...
Real DL = 0.8;                                /* box length. */
Real DH = 1.0;                                /* box height. */
Real ball_radius = resolution_ref * 100.0;    /* ball radius. */
Vec2d ball_center(0.5 * DL, 0.75 * DH);       /* ball center. */
Real cylinder_radius = resolution_ref * 20.0; /* cylinder radius. */
Vec2d cylinder_1_center(0.4 * DL, 0.25 * DH); /* cylinder 1 center. */
Vec2d cylinder_2_center(0.6 * DL, 0.25 * DH); /* cylinder 2 center. */
...
class Ball : public MultiPolygonShape
{
  public:
    ...
    {
        multi_polygon_.addACircle(ball_center, ball_radius, 200, ShapeBooleanOps::add);
    }
};
class CylinderOne : public MultiPolygonShape
{
  public:
    explicit CylinderOne(const std::string &shape_name) : MultiPolygonShape(shape_name)
    {
        multi_polygon_.addACircle(cylinder_1_center, cylinder_radius, 100, ShapeBooleanOps::add);
    }
};
class CylinderTwo : public MultiPolygonShape
{
  public:
    explicit CylinderTwo(const std::string &shape_name) : MultiPolygonShape(shape_name)
    {
        multi_polygon_.addACircle(cylinder_2_center, cylinder_radius, 100, ShapeBooleanOps::add);
    }
};
//----------------------------------------------------------------------
...
int main(int ac, char *av[])
{
    ...
    //----------------------------------------------------------------------
    InnerRelation ball_inner(ball);
    SurfaceContactRelation ball_cylinder_contact(ball, {&cylinder_1, &cylinder_2});
    ...
    InteractionDynamics<solid_dynamics::ContactDensitySummation> ball_update_contact_density(ball_cylinder_contact);
    InteractionDynamics<solid_dynamics::ContactForceFromWall> ball_compute_solid_contact_forces(ball_cylinder_contact);
    //----------------------------------------------------------------------
    ...
    while (GlobalStaticVariables::physical_time_ < end_time)
    {
        ...
        while (integration_time < output_interval)
        {
            ...
            while (relaxation_time < Dt)
            {
                ...
                ball_cylinder_contact.updateConfiguration();
